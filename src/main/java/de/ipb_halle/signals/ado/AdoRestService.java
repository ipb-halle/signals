...
public class AdoRestService implements RestReplyParser<Ado> {
    ...
    private JsonObject buildAdoJsonPayload(Ado ado, String code) {
        ...
        meta.addProperty("adoTypeName", "IPB Code");
        data.add("meta", meta);
    ...
    //toDo: refactor Ado build Attributes to array . error 400 ctyo=+-q8ik
    /**
     * RestReply{string='{"errors":[{"status":"400","code":"BadRequest","title":"The request parameters are in bad format.","detail":"[ERROR - [Path '/data/attributes/fields'] Instance type (object) does not match any allowed primitive type (allowed: [\"array\"]): []]"}]}', path=null, digest='null', mimeType='application/vnd.api+json', fileSize=null, type=STRING}
